...
def main():
    ...
    parser.add_argument(
        "--backend", choices=["AUTO", "TRITON", "MARLIN", "CUDA", "SGLANG", "VLLM"]
    )
    ...
    if backend == BACKEND.SGLANG:
        ...
        subprocess.check_call(
            [sys.executable, "-m", "pip", "install", "sglang[srt]>=0.3.2"]
        )
    ...
    if backend == BACKEND.SGLANG or backend == BACKEND.VLLM:
        ...
        if backend == BACKEND.SGLANG:
            ...
            subprocess.check_call(
                [sys.executable, "-m", "pip", "install", "vllm>=0.8.5"]
            )
            subprocess.check_call(
                [sys.executable, "-m", "pip", "install", "sglang[srt]>=0.3.2"]
            )
            ...
            output = (
                model.generate(prompts=prompt, temperature=0.8, top_p=0.95)[0]
                .outputs[0]
                .text
            )
            model.shutdown()
        ...
        examples = [
            tokenizer("I am in Paris and I can't wait to explore its beautiful")
        ]
