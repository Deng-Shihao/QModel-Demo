...
def _summarize_module_tensors(mod: nn.Module, *, recurse: bool = False):
    ...
    for t in _iter_tensors():
        ...
        dev_key = (
            "meta" if is_meta else (str(t.device) if hasattr(t, "device") else "-")
        )
        n = t.numel()
        total_elems += n
    ...
    if not dtype_set:
        ...
        dnames = [
            (str(d).replace("torch.", "")) if d is not None else "-"
            for d in list(dtype_set)[:3]
        ]
        dtype_str = (
            "mixed[" + ", ".join(dnames) + ("" if len(dtype_set) <= 3 else ", …") + "]"
        )
...
def _annotate(mod: nn.Module, *, color: bool = True) -> str:
    device_str, dtype_str, _n, alloc_b, est_b = _summarize_module_tensors(
        mod, recurse=False
    )
...
def _format_line(
    ...
    if show_counts:
        ...
        counts = _maybe(
            f"(P={human_count(p)} B={human_count(b)})", FG_YELLOW, color=color
        )
...
def _print_params(indent: str, mod: nn.Module, *, include_buffers: bool, color: bool):
    def _line(kind: str, name: str, t: torch.Tensor) -> str:
        ...
        esize = (
            t.element_size()
            if (
                not getattr(t, "is_meta", False)
                and hasattr(t, "device")
                and t.device.type != "meta"
            )
            else _elem_size(getattr(t, "dtype", None))
        )
    ...
    *,
        ...
        try:
            ...
            if len(parts) >= 2:
                ...
                if dev.startswith("mixed[") and dev.endswith("]"):
                    items = dev[6:-1]
                    colored = ", ".join(
                        color_device(s.strip()) for s in items.split(",")
                    )
                ...
                if dt.startswith("mixed[") and dt.endswith("]"):
                    items = dt[6:-1]
                    colored = ", ".join(
                        color_dtype(s.strip()) for s in items.split(",")
                    )
    ...
    def print_params_with_colors(indent: str, mod: nn.Module, *, include_buffers: bool):
        ...
        def _line(kind: str, name: str, t: torch.Tensor) -> str:
            ...
            size_y = _maybe(
                _human_bytes(sizeb), FG_YELLOW, color=color
            )  # size (yellow)
    ...
    def _format_line(
        ...
        name = _maybe(
            qual_name, depth_color(depth), color=color
        )  # module name (depth-based color)
        ...
        for i, (child_name, child) in enumerate(children):
            ...
            if should_collapse(display_name, child):
                ...
                if k_show < total_k and total_k > 0:
                    ...
                    print(
                        _maybe(
                            child_prefix + ("   " if last else "│  ") + collapsed,
                            DIM,
                            color=color,
                        )
                    )
...
def _ensure_target_storage_on_device_(
    param: torch.nn.Parameter, device: torch.device
) -> torch.nn.Parameter:
    ...
    param.data = param.data.to(
        device, copy=True
    )  # alloc new storage on device; keeps Parameter identity
    return param
    ...
    with torch.inference_mode():
        for name, s_p in s_params.items():
            ...
            if t_p_new is not t_p:
                ...
                t_p = t_p_new
            t_p.detach().copy_(
                s_p.detach(), non_blocking=(non_blocking and s_p.is_pinned())
            )
    ...
    for name, s_b in s_bufs.items():
        ...
        if tb is None or getattr(tb, "is_meta", False) or tb.device.type == "meta":
            ...
            if tb.device != device:
                ...
                new_tb.copy_(
                    s_b.detach(), non_blocking=(non_blocking and s_b.is_pinned())
                )
...
def _is_meta_tensor(t: torch.Tensor) -> bool:
    return bool(getattr(t, "is_meta", False)) or (
        hasattr(t, "device") and t.device.type == "meta"
    )
...
def alias_all_from_turtle_if_meta(
    ...
    *,
        ...
        if verify_shapes:
            ...
            for n in set(shell_ps.keys()) & set(turtle_ps.keys()):
                ...
                for n in set(shell_bs.keys()) & set(turtle_bs.keys()):
                    ...
                    swapped += 1
                    log.info(
                        f"Module: Sync {qname} <- from turtle ({turtle_sub.__class__.__name__})"
                    )
                    continue
            continue
    ...
    if (
        tie_after
        and hasattr(shell_model, "tie_weights")
        and getattr(shell_model.config, "tie_word_embeddings", False)
    ):
        try:
