...
class BaseQuantLinear(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._configure_pack_dtype(pack_dtype)
        ...
        if register_buffers:
            ...
            self._register_quant_buffers(
                in_features=in_features,
                out_features=out_features,
                with_bias=bias,
            )

    def _configure_pack_dtype(self, pack_dtype: t.dtype) -> None:
        """
        Configure tensor dtypes/numpy counterparts used for packing logic.
        Raises if an unsupported dtype is provided.
        """
        ...
        if self.pack_dtype == t.int8:
            ...
            raise ValueError(
                "Unsupported weight_dtype. Only int16, int32, and int64 are supported."
            )

    def _register_quant_buffers(
        self, in_features: int, out_features: int, with_bias: bool
    ) -> None:
        """
        Lazily register quantization buffers so that kernels can reuse this helper.
        """
    ...
    def list_buffers(self) -> List:
        """
        Return registered quantization buffers. The order mirrors initialization.
        """
        buffers = []
        for attr in ("qweight", "qzeros", "scales", "g_idx", "bias"):
    ...
    def qzero_format(self, format: int = None) -> int:
        """
        Getter/setter for qzero storage format. Only GPTQ v1 (1) and v2 (2) are valid.
        """
        # get
        if format is None:
    ...
    def verify_supports_params(cls):
        ...
        missing_variables = base_supports_variables - set(child_supports_variables)
        ...
        for name, value in child_supports_variables.items():
            if value is None:
    ...
    def _validate(
        ...
        if dynamic is not None:
            ...
            dynamic_values = {
                "bits": (cls.SUPPORTS_BITS, bits),
                "group_size": (cls.SUPPORTS_GROUP_SIZE, group_size),
                "sym": (cls.SUPPORTS_SYM, sym),
                "act_order": (cls.SUPPORTS_ACT_ORDER, act_order),
            }
            for key, (supported, default) in dynamic_values.items():
                for layer, value in cls._extract_dynamic_param(
                    dynamic, key, default
                ).items():
                    if value not in supported:
                        err = (
                            f"{cls} only supports `{supported}` {key}: actual {key} = `{value}` "
                            f"for layer `{layer}`"
                        )
                        return False, NotImplementedError(err)
    ...
    @staticmethod
    def _extract_dynamic_param(dynamic: dict, key: str, default):
        """
        Helper to obtain per-layer overrides from dynamic quantization configs.
        """
        if not dynamic:
            return {}
        return {
            pattern: params.get(key, default) for pattern, params in dynamic.items()
        }
...
class PackableQuantLinear(BaseQuantLinear):
    def post_init(self, **kwargs):
        ...
        self.register_buffer(
            "wf_unsqueeze_zero",
            wf.unsqueeze(0).to(device=self.g_idx.device),
            persistent=False,
        )
        self.register_buffer(
            "wf_unsqueeze_neg_one",
            wf.unsqueeze(-1).to(device=self.g_idx.device),
            persistent=False,
        )
    ...
    def list_buffers(self):
        ...
        if (
            hasattr(self, "wf_unsqueeze_neg_one")
    ...
    def pack_block(
        ...
        if g_idx.numel() != in_features:
            raise ValueError(
                f"g_idx length {g_idx.numel()} != in_features {in_features}"
            )
        g_idx = g_idx.to("cpu")
        ...
        if linear.bias is not None:
            self.register_buffer(
                "bias", linear.bias.detach().to("cpu", dtype=t.float16)
            )
        ...
        if not disable_ext and bits in (2, 4, 8):
            try:
                ...
                if force_ext:
                    raise
                log.debug(
                    "pack_block: native extension unavailable, falling back to Python path (%s)",
                    exc,
                )
        ...
        if OUT_DTYPE != t.int32:
            raise ValueError(
                "pack_block() expects self.pack_dtype == torch.int32 for 32-bit words."
            )
        ...
        if bits in (2, 4, 8):
            shifts_pf64 = (t.arange(pack_factor, dtype=t.int64) * bits).view(
                1, pack_factor, 1
            )  # [1,pf,1]

        # ---------- pack helpers (int64 shifts -> mask -> int32) ----------
        @t.inference_mode()
        def _pack_rows_2_4_8(
            int32_blk_32xN: t.Tensor, dst: t.Tensor, dst_rows_base: int
        ):
            # int32_blk_32xN: [32, N]
        ...
        def _pack_rows_3(int32_blk_32xN: t.Tensor, dst: t.Tensor, dst_rows_base: int):
            ...
            A = (
                (x[0:10] << (t.arange(10, dtype=t.int64).view(-1, 1) * 3)).sum(
                    dim=0, dtype=t.int64
                )
            ) & MASK32
            ...
            B = (
                B
                | (
                    (x[11:21] << (t.arange(10, dtype=t.int64).view(-1, 1) * 3 + 1)).sum(
                        dim=0, dtype=t.int64
                    )
                )
            ) & MASK32
            ...
            C = (
                C
                | (
                    (x[22:32] << (t.arange(10, dtype=t.int64).view(-1, 1) * 3 + 2)).sum(
                        dim=0, dtype=t.int64
                    )
                )
            ) & MASK32
        ...
        if workers_eff == 1:
            ...
            with ThreadPoolExecutor(
                max_workers=workers_eff, thread_name_prefix="pack qweight"
            ) as ex:
        ...
        zeros_np = (
            zeros.detach().cpu().numpy().astype(self.pack_np_math_dtype, copy=False)
        )  # [G, out]
        qzeros_np = np.zeros(
        ...
        self.register_buffer(
            "qzeros", t.from_numpy(qzeros_np.astype(self.pack_np_dtype, copy=False))
        )
    ...
    def pack_gpu(
        ...
        *,
        ...
        if not t.cuda.is_available():
            raise RuntimeError(
                "pack_gpu requires CUDA availability but none was detected."
            )
        ...
        if linear.bias is not None:
            self.register_buffer(
                "bias", linear.bias.detach().to(device="cpu", dtype=t.float16)
            )
        else:
            self.bias = None
        ...
        if bits in (2, 4, 8):
            ...
            shifts_pf64 = (
                t.arange(pack_factor, dtype=t.int64, device=target_device).view(
                    1, pack_factor, 1
                )
                * bits
            )
        ...
        def _pack_rows_2_4_8(
            ...
            dst[dst_rows_base : dst_rows_base + bits] = (packed64 & mask_tensor).to(
                self.pack_dtype
            )
        ...
        def _pack_rows_3_values(int32_blk_32xN: t.Tensor) -> t.Tensor:
            ...
            B = (
                B | ((x[11:21] << (range10 * 3 + 1)).sum(dim=0, dtype=t.int64))
            ) & mask_tensor
            B = (B | ((x[21] << 31) & mask_tensor)) & mask_tensor

            C = ((x[21] >> 1) & 0x3) & mask_tensor
            C = (
                C | ((x[22:32] << (range10 * 3 + 2)).sum(dim=0, dtype=t.int64))
            ) & mask_tensor
        ...
        if bits in (2, 4, 8):
            ...
            zeros_view = zeros_int.view(
                zeros_int.shape[0], zeros_int.shape[1] // pack_factor, pack_factor
            )
            shifts = (
                t.arange(pack_factor, dtype=t.int64, device=target_device).view(
                    1, 1, pack_factor
        ...
        self.register_buffer(
            "qweight", qweight_dev.to(device="cpu", dtype=self.pack_dtype)
        )
        self.register_buffer(
            "qzeros", qzeros_dev.to(device="cpu", dtype=self.pack_dtype)
        )
    ...
    def pack_original(
        ...
        with THREADPOOLCTL.threadpool_limits(1):
            ...
            int_weight = t.round(
                (W + scale_zeros[self.g_idx].T) / scales[self.g_idx].T
            ).to(t.int32)
            ...
            self.register_buffer(
                "qweight", t.from_numpy(qweight.astype(self.pack_np_dtype))
            )
            ...
            self.register_buffer(
                "qzeros", t.from_numpy(qzeros.astype(self.pack_np_dtype))
            )
        ...
        **kwargs,
        ...
        if register_buffers:
            self.register_buffer(
                "qweight",
                t.zeros(
                    (in_features, out_features // (self.pack_dtype_bits // self.bits)),
                    dtype=self.pack_dtype,
                ),
            )
            self.register_buffer(
