...
def make_dequant_configs(
    block_sizes: List[int], num_warps: List[int], num_stages: List[int]
):
    configs = []
...
def dequant_kernel(
    ...
    if bits == 3:
        ...
        zero_word = tl.load(
            qzeros_ptr + zero_word_idx, mask=xmask, eviction_policy="evict_last"
        )
        ...
        if zero_bit_offset <= 29:
            ...
            next_zero_word = tl.load(
                qzeros_ptr + zero_word_idx + 1, mask=xmask, eviction_policy="evict_last"
            )
            combined = (zero_word >> zero_bit_offset) | (
                next_zero_word << (32 - zero_bit_offset)
            )
    ...
    if bits == 3:
        ...
        weight_word = tl.load(
            qweight_ptr + weight_word_idx, mask=xmask, eviction_policy="evict_last"
        )
        ...
        if weight_bit_offset <= 29:
            ...
            # 3-bit value spans two 32-bit words
            next_weight_word = tl.load(
                qweight_ptr + weight_word_idx + 1,
                mask=xmask,
                eviction_policy="evict_last",
            )
            combined = (weight_word >> weight_bit_offset) | (
                next_weight_word << (32 - weight_bit_offset)
            )
...
def quant_matmul(
    input, qweight, scales, qzeros, g_idx, bits, pack_bits, maxq, transpose=False
):
...
class QuantLinearFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, qweight, scales, qzeros, g_idx, bits, pack_bits, maxq):
        output = quant_matmul(
            input, qweight, scales, qzeros, g_idx, bits, pack_bits, maxq
        )
