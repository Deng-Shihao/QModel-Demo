...
def convert_hf_params_to_vllm(hf_params: Dict[str, Any]):
    ...
    if hf_params.get("max_length", None):
        raise ValueError(
            "vLLM does not support argument `max_length`. Please use `max_new_tokens` instead."
        )
    if hf_params.get("min_length", None):
        raise ValueError(
            "vLLM does not support argument `min_length`. Please use `min_new_tokens` instead."
        )
...
def vllm_generate(model, **kwargs):
    ...
    if not isinstance(sampling_params, SamplingParams):
        ...
        sampling_params = convert_hf_params_to_vllm(
            {k: v for k, v in hf_params.items() if v is not None}
        )

    # Convert prompts to vLLM format
    if isinstance(prompts, torch.Tensor):
        tokens_prompts = [
            TokensPrompt(prompt_token_ids=prompt) for prompt in prompts.tolist()
        ]
        req_results = model.generate(
            prompts=tokens_prompts, sampling_params=sampling_params
        )
        ...
        raise ValueError(
            f"Invalid input type for vllm_generate, type is {type(prompts)}"
        )
    ...
    padded_list = [
        sublist + [pad_token_id] * (max_length - len(sublist)) for sublist in outputs
    ]
