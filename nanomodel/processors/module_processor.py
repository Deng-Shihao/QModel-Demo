...
class TqdmProgress:
    ...
    def __init__(
        ...
        *,
        ...
        self._tqdm = tqdm(total=computed_total, desc=desc, unit=unit, leave=False)
        self._displayed = 0
    ...
    def _increment(self, amount: int):
    ...
    def _sync_progress(self):
        target = max(int(self.current_iter_step), 0)
...
class ModuleProcessor():
    ...
    def _run_forward_batches(
        ...
        *,
        ...
        need_outputs: bool,
        reuse_kv: bool,
        progress_pb: TqdmProgress | None = None,
    ...
    def cache_inputs(self, layers, calibration_data, use_cache):
        ...
        if calibration_batches is None:
            logger.info("ModuleLooper: capturing layer inputs (batch count unknown)")
        else:
            logger.info(
    ...
    def loop(self, fail_safe: bool = False, **kwargs):
        ...
        for layer_index in pb:
            ...
            for p_index, processor in enumerate(self.processors):
                ...
                for index, names in enumerate(modules):
                    ...
                    forward_pb = (
                        create_progress(range(forward_total_rows), unit="row")
                        .manual()
                ...
                if not is_last_module and processor.fwd_after_process:
                    ...
                    replay_pb = (
                        create_progress(range(replay_total_rows), unit="row")
                        .manual()
                        .set(show_left_steps=False)
                    )
                ...
                if p_index == len(self.processors) - 1:
                    ...
                    finalize_futures = []
                    finalize_pb = (
                        create_progress(range(finalize_count), unit="module")
                        .manual()
                        .set(show_left_steps=False)
                    )
        ...
        process_finalize_pb = (
            create_progress(range(process_finalize_total), unit="processor")
            .manual()
            .set(show_left_steps=False)
