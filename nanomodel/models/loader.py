...
from ..quantization.config import KERNEL, METHOD, MIN_VERSION_WITH_V2
        ...
        if qcfg.kernel in [KERNEL.GPTQ, KERNEL.GEMM]:
            ...
            if qcfg.kernel == KERNEL.GPTQ and preload_qlinear_kernel.REQUIRES_FORMAT_V2:
                if not qcfg.is_quantized_by_v2():
                    if not qcfg.sym:
                        raise ValueError(
                            f"Format: Loading of a sym=False model with format={KERNEL.GPTQ} is only supported if produced by nanomodel version >= {MIN_VERSION_WITH_V2}"
                        )
                    raise ValueError(
                        f"Format: {preload_qlinear_kernel.__name__} requires checkpoints generated by nanomodel version >= {MIN_VERSION_WITH_V2}"
                    )
